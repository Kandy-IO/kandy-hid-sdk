...
The local library must then be initialized with an object containing keys `mode` and `driverInfo`. Mode can be either `'VDI'` or `'desktop'`. The value for `driverInfo` is the object returned from issuing `getInfo()` on the vchannel in the Main process. `driverInfo` is only required when running in VDI mode, and can be ignored in Desktop mode.
...
This is included for backwards-compatibility only. Devices are closed during app exit.
...
- The Jabra Speak 750 cannot currently be used on a Mac in Desktop or VDI. A support ticket (299) has been created with the device vendor
...
- In eLux VDI, it has been found that connecting a HID device to the Thin Client may cause eLux to select it as the system default device. As well, the previously selected device may be muted. Both of these changes affects in-progress or future calls until rectified. Resolution is to unmute and re-select devices in the eLux system menu. This is not related to the Kandy HID libraries but standard eLux behaviour.
- It has been found that the Jabra Engage 50 does not respond correctly to the 'stop_ringing' instruction; the headset stops blinking, but the Link device continues to blink indefinitely. The only known way to resolve the situation is to unplug and reconnect the device from the host PC. This issue affects all configurations when firmware 2.3.1 is used. Issue is tracked by KAJ-700. A support ticket has been created with the device vendor (724).
